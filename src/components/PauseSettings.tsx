  ...
  return (
    <div className="pause-settings-overlay">
      <section className="pause-settings" role="dialog" aria-modal="true" aria-labelledby="pause-settings-title">
        <header>
          <div><p className="eyebrow">ПАРАМЕТРЫ МАТЧА</p><h2 id="pause-settings-title">Настройки</h2></div>
          <button type="button" className="pause-settings__close" onClick={onClose} aria-label="Закрыть настройки">×</button>
        </header>
        <div className="pause-settings__list">
          <VolumeControl label="Игровые звуки" value={options.volume} onChange={(value) => {
            onOptionChange('volume', value);
            if (!options.effectsEnabled) onOptionChange('effectsEnabled', true);
          }} />
          <VolumeControl label="Комментатор" value={options.commentaryVolume} disabled={!options.commentaryEnabled} onChange={(value) => onOptionChange('commentaryVolume', value)} />
          <VolumeControl label="Болельщики" value={options.crowdVolume} onChange={(value) => {
        ...
        <button type="button" className="pause-settings__done" onClick={onClose}>Готово</button>
      </section>
    </div>
  );
}
