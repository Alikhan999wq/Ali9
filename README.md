...
Полноценная 2D-футбольная игра, встроенная в приложение **Vite + React + TypeScript**.

- Главная страница: `/`
- Игра: `/game`
- Управление: `WASD`, `Space`, `P`
- Запуск: `npm install`, затем `npm run dev`

Игровой движок и физика находятся в `src/game/`, интерфейс — в `src/components/`,
а отдельный игровой экран — в `src/pages/GamePage.tsx`.

В игре реализованы 90-секундные матчи, три команды, три уровня ИИ, физика мяча,
табло, полная итоговая статистика, судья, фолы, карточки, VAR-объяснения,
процедурные звуки, русскоязычный комментатор, динамическая атмосфера настоящих
футбольных трибун, мобильное управление и сохраняемые настройки качества,
языка и громкости.

## Звуки трибун

Записи находятся в `public/assets/sounds/crowd/`. Фоновая атмосфера смешивает
две длинные полевые записи, усиливается во время опасной атаки, отдельно
реагирует на гол и плавно затихает после финального свистка. В настройках
громкость трибун регулируется независимо, звук можно полностью отключить.
Громкость комментатора, атмосферы и игровых эффектов имеет отдельные ползунки
и выключатели. Во время реплики комментатора трибуны плавно приглушаются на
25%, не прерываясь, а затем возвращаются к текущей игровой интенсивности.

Использованные полевые записи распространяются под лицензией Creative Commons
Zero (CC0):

- Sandermotions — `Soccer Stadium 10.wav`
- ckater — `football game in a big arena`
- Kinoton — `Crazy Soccer Crowd Cheering`
- Eightmate — `Local Football Crowd Cheering`
